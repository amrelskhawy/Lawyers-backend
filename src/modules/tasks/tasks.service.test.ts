...
describe("stand-in assignees", () => {
    const TEMP = "user-temp";

    it("stores stand-ins alongside owners when the switch is on", async () => {
        prismaMock.default.task.create.mockResolvedValue({});
        prismaMock.default.user.count.mockResolvedValue(2);

        await TasksService.create(ME_USER, {
            title: "t",
            assigneeIds: [OTHER],
            hasTempAssignee: true,
            tempAssigneeIds: [TEMP],
        });
        ...
        expect(prismaMock.default.task.create).toHaveBeenCalledWith(
            expect.objectContaining({
                data: expect.objectContaining({
                    ...
                    assignees: {
                        create: [
                            { userId: OTHER, isTemp: false },
                            { userId: TEMP, isTemp: true },
                        ],
                    },
                }),
            }),
    ...
    it("ignores the stand-in list while the switch is off", async () => {
        prismaMock.default.task.create.mockResolvedValue({});
        prismaMock.default.user.count.mockResolvedValue(1);
        ...
        expect(prismaMock.default.task.create).toHaveBeenCalledWith(
            expect.objectContaining({
                data: expect.objectContaining({
                    hasTempAssignee: false,
                    assignees: { create: [{ userId: OTHER, isTemp: false }] },
                }),
            }),
        );
    });

    it("keeps someone named on both lists as an owner — one row per user", async () => {
        prismaMock.default.task.create.mockResolvedValue({});
        prismaMock.default.user.count.mockResolvedValue(1);
    ...
    it("turning the switch off drops the stand-ins but keeps the owners", async () => {
        prismaMock.default.task.findUnique.mockResolvedValue({
            createdById: ME,
            hasTempAssignee: true,
            assignees: [
                { userId: OTHER, isTemp: false },
        ...
        await TasksService.update("task-1", ME_USER, { hasTempAssignee: false });

        expect(prismaMock.default.task.update).toHaveBeenCalledWith(
            expect.objectContaining({
                data: expect.objectContaining({
                    assignees: { deleteMany: {}, create: [{ userId: OTHER, isTemp: false }] },
                }),
            }),
        );
    });

    it("replacing only the stand-ins leaves the owners in place", async () => {
        prismaMock.default.task.findUnique.mockResolvedValue({
            createdById: ME,
            hasTempAssignee: true,
            assignees: [
        ...
        await TasksService.update("task-1", ME_USER, { tempAssigneeIds: [ME] });
